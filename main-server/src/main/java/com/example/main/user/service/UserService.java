...
import com.example.main.global.util.JwtProvider;
import com.example.main.refresh.service.RefreshTokenService;
import com.example.main.user.dto.*;
...
public class UserService {
    ...
    public UserProfileResponseDto updateProfile(UserProfileUpdateRequestDto requestDto,
        ...
        user.updateProfile(requestDto.getNewName(),newPassword);

        if(requestDto.getNewAddress() != null) {
            UserProfileUpdateRequestDto.AddressUpdateDto newAddress = requestDto.getNewAddress();
            user.getAddress().update(newAddress.getReceiverName(),newAddress.getPhoneNumber(), newAddress.getPostCode(), newAddress.getRoadAddress(), newAddress.getDetailAddress());
        }
